				...
				<div v-if="products.length > 0" class="mb-4 grid gap-4 sm:grid-cols-2 md:mb-8 lg:grid-cols-3 xl:grid-cols-4 mt-8">
                    <div v-for="product in products" :key="product.id" class="relative rounded-3xl border border-gray-200 bg-white p-6 shadow-sm dark:border-neutral-700 dark:bg-neutral-800">
                        <div class="pt-6">
                            ...
                            method="delete"
                            as="button"
                            class="absolute top-4 right-4 rounded-lg border border-red-200 bg-red-100 px-3 py-2 text-sm font-medium text-red-600 hover:bg-red-200 hover:text-red-800 focus:z-10 focus:outline-none focus:ring-4 focus:ring-red-100 dark:border-red-600 dark:bg-red-800 dark:text-red-200 dark:hover:bg-red-700 dark:hover:text-white dark:focus:ring-red-700"
                            >
                                <Trash2 />
                            </Link>
                            ...
                            <div class="flex items-center gap-4 mt-6">
                                <Link :href="route('products.edit', product.id)" method="get" as="button" class="rounded-lg border border-gray-200 bg-white px-5 py-2.5 text-sm font-medium text-gray-900 hover:bg-gray-100 hover:text-primary-700 focus:z-10 focus:outline-none focus:ring-4 focus:ring-gray-100 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white dark:focus:ring-gray-700">
                                    <Pencil />
                                </Link>
                            </div>
                        </div>
